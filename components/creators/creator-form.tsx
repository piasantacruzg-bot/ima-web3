...
import { AvatarUpload } from "@/components/creators/avatar-upload";
import { SocialAccountsField } from "@/components/creators/social-accounts-field";
...
export function CreatorForm({
  ...
  checkDuplicates: (formData: FormData) => Promise<DuplicateCandidate[]>;
  creator?: Creator;
  submitLabel?: string;
}) {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    const found = await checkDuplicates(formData);
    setChecking(false);
    if (found.length > 0) {
      setCandidates(found);
    } else {
  ...
  return (
    <>
      ...
      <form ref={formRef} action={formAction} onSubmit={handleSubmit} className="space-y-8">
        {creator ? <input type="hidden" name="creator_id" value={creator.id} /> : null}
        ...
        <section>
          ...
          <AvatarUpload initialUrl={creator?.profile_image_url ?? undefined} />
        </section>
        ...
        <section className="grid grid-cols-2 gap-4">
          ...
          <div>
            <label className="label" htmlFor="state_province">
              State / Province
            </label>
            <input
              id="state_province"
              name="state_province"
              defaultValue={creator?.state_province ?? ""}
              className="input"
            />
          </div>
          <div>
        ...
        <section>
          ...
          <textarea id="bio" name="bio" rows={3} defaultValue={creator?.bio ?? ""} className="input" />
        </section>
        ...
        <section>
          ...
          {creator ? (
            <p className="mt-1 text-xs text-ink-soft">
              This adds new accounts — existing ones aren&apos;t editable here yet.
            </p>
          ) : null}
        </section>

        <SubmitButton label={checking ? "Checking for duplicates…" : submitLabel} />
      </form>
    </>
  );
}
